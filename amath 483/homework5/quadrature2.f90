...
real(kind=8 )function simpson(f,a,b,n)
    ...
    !   n:  number of points to use
    ! Returns:
    !	integral of f over a to b = (b-a)/6 * (f(a) + 4*f((a+b/2)) + f(b))
    !   the estimate of the integral
    ...
    ! Local variables:
    integer :: j
    real(kind=8) :: h, simp_sum, x_j, x_c
    simp_sum = f(b) - f(a)

    h = (b-a)/(n-1)

    !$omp parallel do private(x_j,x_c) reduction(+ : simp_sum) 
    do j= 1,n-1
        x_j = a + h*(j-1)
        x_c = a + h/2.d0 + (j-1)*h
        simp_sum = simp_sum  + 2.d0 * f(x_j)  + 4.d0 * f(x_c)
        enddo
    simpson = h/6.d0 * simp_sum
